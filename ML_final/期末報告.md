...
$$
L_i(\theta_i) = \mathbb{E}_{(\phi, a, r, \phi') \sim U(D)} \left[ \left( \underbrace{\left( r + \gamma \max_{a'} \hat{Q}(\phi', a'; \theta_i^-) \right)}_{\text{由 Target Network 計算}} - \underbrace{Q(\phi, a; \theta_i)}_{\text{由 Main Network 計算}} \right)^2 \right]
$$
